...
exports.findAll = async (req, res) => {
  ...
  await Ocorrencias.find(condition)
    .then((data) => {
      res.send(data)
    })
    .catch((err) => {
...
exports.update = (req, res) => {
  ...
  Ocorrencias.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
    .then((data) => {
      if (!data) {
        res.status(404).send({
...
exports.delete = (req, res) => {
  ...
  Ocorrencias.findByIdAndRemove(id, { useFindAndModify: false })
    .then((data) => {
      if (!data) {
        res.status(404).send({
...
exports.findAllPublished = (req, res) => {
  Ocorrencias.find({ published: true })
    .then((data) => {
      res.send(data)
    })
    .catch((err) => {
